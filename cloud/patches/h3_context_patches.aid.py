...
def release_continuity_patches():
    """Restore only wrappers owned by this Director invocation.

    T8's normal Hybrid path validates the core functions. Leaving Director's
    global wrappers installed after a long-video task poisons the next ordinary
    i2v task even though no core file has changed. Never unwrap a foreign patch.
    """
    global _layout_orig, _layout_applied, _payload_orig, _payload_applied
    import comfy.model_base as model_base

    mm = _mm()
    if _layout_orig is not None:
        if mm.PackedLayout.__init__ is _director_layout_init:
            mm.PackedLayout.__init__ = _layout_orig
            _layout_orig = None
            _layout_applied = False
        else:
            log.warning("Director cleanup: foreign layout owner; left untouched")
    if _payload_orig is not None:
        if model_base.MiniMaxH3.extra_conds is _director_extra_conds:
            model_base.MiniMaxH3.extra_conds = _payload_orig
            _payload_orig = None
            _payload_applied = False
        else:
            log.warning("Director cleanup: foreign payload owner; left untouched")


def scoped_continuity_patches(function):
    """Scope runtime compatibility patches to a complete Director plan."""
    @wraps(function)
