...
## 2026-09-05 线上采样修正（v0.1.195）

本节替代 v0.1.194 中“尚未进行 30/60 秒 GPU 效果实测”的阶段结论；以下修改只针对独立图生视频页的实验性 Director 长视频，不改变 Story/连续剧的既有短视频流程，也不增加画面或声音质检。

- 同源、同 Seed、同六段提示词的 60 秒受控 A/B 显示：普通 `LoraLoaderModelOnly` 在多段续接后出现严重拖影、爆亮与人脸破坏；改成 `LoraLoaderBypassModelOnly` 后，0/15/25/35/45/50.5/55/59 秒抽帧均保持正常。单段测试不足以暴露这个累计问题。
- AID 正式工作流固定使用 Bypass LoRA；四步 DaSiWa 图向 Director 传递 `euler/simple`，线上旧版 ComfyUI 由可回退兼容层转接至已安装 T8 的 `dual_clock_euler`。兼容层只在 `4 steps + euler/simple + shift 12/3 + full denoise` 的精确合同下启用，其他 Director 模式仍走原实现。
- 文本模型偶尔把第 2–6 段写成全局 `00:10–01:00` 时间码，而 Director 每段只接受本地约 10 秒时间线。提交前现按段确定性归一为 `00:00–00:10`，再由连续上下文逻辑加入 0.917 秒引导区，动作与原台词不被改写。
- 网站默认仍由本机 Companion 安全持有 SSH 密钥并提交连续任务；页面要求 Companion 至少为 v0.1.195，旧版会在付费提交前被拦截，不能再用旧采样配置重建请求。本次同步发布网站、Companion 与 GPU 后端。
- 正式线上补丁另跑了一次全新 10 秒真实烟测：任务 `1d4bf093-5b2a-4a4a-82e5-ac827b31c3ad`，输出 10.125 秒、480×864 H.264、32kHz 双声道 AAC，服务端日志确认命中 T8 双时钟路径，5 秒抽帧无此前的拖影爆亮。样片保存在 `outputs/h3-director-diagnosis-20260905/production-patch-smoke.mp4`。
- 音轨确认完整且可解码，但机器分析不能代替人工听感；本次没有增加 ASR、降噪、尾音或杂音质检，也不声称仅凭容器指标已经证明所有杂音消失。修正发生在生成源头：LoRA/音视频联合采样与分段时序。

生产 GPU 的 Director 从原提交 `9e6b4fbe9011d41434a6c3b3757c40a5429cf278` 切到可回退分支 `codex/aid-production-v0.1.195`，补丁提交 `30124a8e9e6ab1c27672160db8b8ae91dfb061ad`。仓库内的部署副本为 `cloud/patches/core_sampling.aid.py`。
