...
try {
  ...
  const { project } = await request({ action: 'create', project: { name: '回声档案 · 流程验证', brief: '照片重现十秒往事，调查以记忆为代价。（测试数据）', episodeCount: 3 } });
  ...
  await request({ action: 'finish', jobId: running.job.id, lease: running.job.lease, paused: true });
  ...
  execFileSync(require('ffmpeg-static'), ['-hide_banner', '-loglevel', 'error', '-f', 'lavfi', '-i', 'color=c=0x252036:s=180x320:r=1:d=120', '-f', 'lavfi', '-i', 'anullsrc=r=8000:cl=mono', '-t', '120', '-c:v', 'libx264', '-preset', 'ultrafast', '-c:a', 'aac', '-movflags', '+faststart', file]);
  ...
  console.log(JSON.stringify({ ok: true, seriesId, checks: ['deduplication', 'exclusive lease', 'stale lease rejection', 'credential redaction', 'pause/resume', 'validated video upload', 'range playback', 'download', 'revision conflict', 'downstream invalidation', 'old delivery retention'] }, null, 2));
