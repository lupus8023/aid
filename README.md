...
Story 长片生成采用分阶段流程：一句话构想也可以先扩写为完整的故事目标、冲突、转折、选择与结局，再锁定全片故事骨架和精确镜头数，按场次以最多 9 镜一批展开详细剧本，最后按相同边界生成导演分镜提示词。每个出场角色都会获得逐镜的走位、手势、微表情、视线、呼吸、即时反应、潜台词与逐字台词；这些表演卡会在分镜审核页展示，并继续进入视频提示词。批次之间会传递人物、道具、关系和物理状态；切换场次时允许明确重建时间、空间和光线，避免前后剧情漂移。
...
视频阶段先把连续分镜组织为最长 15 秒的 H3 片段，再由片段锁定唯一的有序台词表；分镜只保留各自的时间窗、完整动作、表情、景别、走位、动作声和电影化镜头衔接。同一片段可以由多个不同人物依次说话，但每个人物只有一个连续发声块，分镜切换不会再创建第二条台词。首尾帧参考只锁定连续性与动作落点，不会把整段视频均匀拉成慢动作。

有台词的 Story 片段使用单次 H3 原生音画流程：提示词把最高优先级的音频事件锁和逐字对白事件放在最前，逐字台词只在顶层事件的官方 `d` 标签中出现一次；后续时间轴按秒连续覆盖完整片段，并为每个区间明确镜头、动作、表情、对白状态、景别机位、人物关系和走位。时间轴只通过事件 ID 引用对白，其他 JSON 字段均为不可朗读的导演控制数据。同一人物在一个片段中只能有一段连续台词，相邻短句会合并成长台词，避免台词间隙触发续说、杂音或参考样本泄漏。角色音频只绑定音色，动作、结果或观众反应不会重复解释台词含义。成片直接采用主 H3 同步音轨，不再额外生成台词、重混人声、做人声分离或为环境声重复运行完整 H3。第一句前保留干净场景底噪，最后一句预留完整尾字和自然环境尾音。
