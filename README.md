...
视频阶段默认采用保真优先策略：每张分镜图独立生成一个视频片段。普通分镜把该图直接接入 H3 的 `first_frame`，以 I2VA 锁定第 0 秒真实画面；承接上一镜时使用上一段运动尾帧与当前目标帧组成 FL2VA。自动流程不再把多张分镜图合并成 Ref2VA 后要求模型重新综合构图；确有压缩片段需求时仍可在片段编辑中手动合并。提示词会锁定人物身份、服装、场景拓扑、材质、光线、镜头透视和色彩，只允许剧本明确要求的动作、微表情、视线、呼吸、运镜和物理效果发生变化。
