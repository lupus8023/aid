...
interface Input { voiceId: string; fishAudioKey: string; language: 'zh' | 'en'; strictVoice: boolean }
interface Cached { audio: string; voiceId: string; url?: string; duration?: number }
...
export function createVoiceReferenceService(deps: Dependencies) {
  const pending = new Map<string, Promise<{ url: string; voiceId: string; duration: number }>>();
  return async (input: Input) => {
    ...
    const operation = (async () => {
      ...
      try {
        ...
        if (cached?.url) return { url: cached.url, duration: cached.duration || 0, voiceId: cached.voiceId };
        await deps.ready();
      } catch (error) {
      ...
      try {
        ...
        await save();
        return { url: cached.url, duration: cached.duration || 0, voiceId: cached.voiceId };
      } catch (error) {
...
export function generateVoiceReference(input: Input) {
  service ||= createVoiceReferenceService({
    ...
    ready: ensureCloudinaryUploadReady, synthesize: generateFishSpeech, upload: uploadBufferToCloudinary,
  });
