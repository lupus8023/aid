...
export function seriesPrompt(
  ...
  const sourceAuthority = authored
    ? `用户输入是已经完成导演设计的权威成稿，不是供改编的故事素材。禁止新增、删除、合并、拆分、调序或替换事件和镜头；禁止续写前史、后续、支线、角色、对白或结局。每镜的动作、景别、运镜、氛围、AI生图提示词和逐字台词都是锁定字段。文本模型只可：提取角色/场景/道具资产、把正名映射为ID、补齐结构化技术字段，以及在原时长客观说不完原台词时仅延长该镜时长。原稿镜头合同：${JSON.stringify(authored.shots)}`
