...
为两分钟视觉叙事控制出场人数和场景，不靠长篇解说。遵守用户的类型与人物设定。`;
  if (stage === "outline")
    return `${base}
只生成整季总纲、全季已知的角色与场景。不写所有分镜。设计能够反复产生新冲突的机制。阶段arcs必须从第1集连续覆盖到最后一集。所有重要谜团必须有具体答案、埋设集数与回收集数。角色别名不能与其他角色姓名/别名重复。旁白/电话声音也列入人物，appearance=voice_only。公共场景重用但场景状态可随剧情改变。
