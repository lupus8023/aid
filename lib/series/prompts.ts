...
主角与主要配角应可在视觉和声音上区分。所有已知的发声配角在这里登记，避免分集新增无档案人物。`;
  if (stage === "episodes") {
    ...
    return `${base}
权威总纲与固定编号：${JSON.stringify({ bible: project.bible, characters: project.characters.map(({ id, name, role, want, secret, arc, appearance }) => ({ id, name, role, want, secret, arc, appearance })), locations: project.locations.map(({ id, name, description }) => ({ id, name, description })) })}
...
每镜最多3轮台词。允许甲→乙→甲的自然短对答，必须保持轮次顺序；同一角色紧接着连续说的句子合并为一轮，不得将隔着他人回答的句子提前合并或调序。台词更多时在相邻镜头规划完整交流，但仍严格保持18镜与总时长。
返回：{"shots":[{"number":1,"seconds":7,"locationId":"l1","characterIds":["c1"],"visual":"景别、构图和可见内容","action":"动作及人物反应","dialogue":[{"characterId":"c1","text":"具体台词","emotion":"表演语气"}],"sound":"环境/音效，不含新增对白","purpose":"此镜造成的信息/关系/局面变化"}]}`;
}
